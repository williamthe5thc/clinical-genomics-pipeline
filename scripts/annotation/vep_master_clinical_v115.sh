...
LOGS_DIR="${BASE_DIR}/logs/annotation_logs"

# VEP configuration
VEP_CACHE_DIR="${DATABASE_DIR}/vep_cache"
VEP_PLUGINS_DIR="${BASE_DIR}/vep_setup/plugins"
...
add_custom_annotations() {
    ...
    # AlphaMissense (2024-2025 gold standard)
    if [[ -f "$ALPHAMISSENSE_FILE" ]]; then
        # Use custom annotation approach (NOT plugin)
        vep_cmd+=" --custom '$ALPHAMISSENSE_FILE',AlphaMissense,bed,overlap,0,"
        vep_cmd+="am_pathogenicity,am_class"
    fi
...
run_master_vep_annotation() {
    ...
    log_info "VEP execution started at $(date)"
